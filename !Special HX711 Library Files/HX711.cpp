 ...
 * History:
 * 7-APR-2022 JMC
 * Added 'const' qualifier to appropriate class methods.
 *
  * MIT License
...
long HX711::read_average(byte times) const {
	long sum = 0;
...
double HX711::get_value(byte times) const {
	return read_average(times) - OFFSET;
}

float HX711::get_units(byte times) const {
...
long HX711::get_offset() const {
	return OFFSET;
}

void HX711::power_down() const {
	digitalWrite(PD_SCK, LOW);
	digitalWrite(PD_SCK, HIGH);
}

void HX711::power_up() const {
	digitalWrite(PD_SCK, LOW);
}
